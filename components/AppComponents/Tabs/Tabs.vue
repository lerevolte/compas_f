        ...
        <ul class="tabs__list">
            <template  v-for="(tab, index) in tabs.list" :key="tab.id">
                <li
                    ...
                    class="tabs__item"
                    :data-tab="tab.tab"
                    :class="{'tabs__item_hidden': !tab.enabled, 'tabs__item_active': (tab.tab == props.activeTab && !props.isModule) || (tab.tab == 'modules' && props.isModule), 'tabs__item_unlocked': props.disableAll && ['order', 'products'].includes(tab.tab)}"
                >
